...
def read_image_list_file(image_id_list):
  for line in image_id_list:
    yield line.replace('.jpg', '')
...
def download_all_images(image_id_list, download_folder, num_processes=5):
  ...
  try:
    image_list = list(
        check_and_homogenize_image_list(
            read_image_list_file(image_id_list)))
